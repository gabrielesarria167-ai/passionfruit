...
`index.html` is the opening scene: a procedural 3D passionfruit (three.js) falls through a dark studio, bursts on the floor and throws a plume of torn rind, golden pulp, seeds and juice into the air. A follow spot picks the fruit out on the way down; after the burst the pulp itself is the light source, so the black floor is lit by what is lying on it. One line waits under the fall, and the line that answers it stands inside the scene, so the pieces thrown up cross in front of the words while the camera closes in on them. As the hold on that line ends the camera sinks and tips its gaze down onto the scatter, so the words rise away out of the frame, and the pieces start to turn, each on its own a moment before it would touch the floor: every drop of juice and piece of pulp lights up and falls into a real star, most of the rind goes the same way and a few pieces come back as comets shooting across the new sky, down and to the right. What is still in the air keeps flying until its turn. Night comes up from below as they go and the floor gives way to it, the rest of the sky comes up brightest first, and the camera settles on Orion. The fruit is generated in code, no model files; the sky is the Yale Bright Star Catalogue.
